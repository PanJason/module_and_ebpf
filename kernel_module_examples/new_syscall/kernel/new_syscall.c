...
#include <linux/kallsyms.h> // For sprint_symbol
 ...
 // up all the memory.
 //
 // This is a parameter provided by the user, like:
 // insmod new_syscall.ko sym=0xffffffff81a001c0
...
{  
    ...
    pr_info("Registering kprobe at %p\n", syscall_kprobe.addr);
    ...
    pr_info("Syscall table found at %px\n", sys_call_table_stolen);
...
MODULE_DESCRIPTION("A simple example to replace a syscall");
MODULE_VERSION("1.0");
